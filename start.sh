...
if ! command -v firefox &> /dev/null; then
    echo "[INFO] Firefox not found. Installing..."
    if [ "$PKG_MGR" == "apt" ]; then
        # Download and install Firefox directly from Mozilla (apt-only provides snap version)
        ...
        sudo tee /usr/share/applications/firefox.desktop > /dev/null << 'FIREFOX_DESKTOP'
[Desktop Entry]
Name=Firefox
...
Terminal=false
MimeType=text/html;text/xml;application/xhtml+xml;application/vnd.mozilla.xul+xml;text/mml;application/pdf;
StartupNotify=true
FIREFOX_DESKTOP
